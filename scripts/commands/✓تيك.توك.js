...
module.exports.handleReply = async function({ api, event, handleReply }) {
	...
	try {
		...
		ytdl(handleReply.link[event.body - 1])
			...
			.on("close", () => {
				if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`).size > 26214400) return api.sendMessage('لا يمكن إرسال الملف لأنه أكبر من 25MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`), event.messageID);
		...
		api.sendMessage("❎غير قادر على معالجة طلبك!", event.threadID, event.messageID);
	}
...
module.exports.run = async function({ api, event, args }) {
	...
	if (args.length == 0 || !args) return api.sendMessage('لا يمكن ترك البحث فارغا!', event.threadID, event.messageID);
	...
	if (urlValid) {
		try {
			...
			ytdl(args[0])
				...
				.on("close", () => {
					if (statSync(__dirname + `/cache/${id}.mp4`).size > 26214400) return api.sendMessage('لا يمكن إرسال الملف لأنه أكبر من 25MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${id}.mp4`), event.messageID);
				...
				.on("error", (error) => api.sendMessage(`حدثت مشكلة أثناء معالجة الطلب، خطأ:\nError: ENOENT: لا يوجد مثل هذا الملف أو الدليل، مفتوحة '/home/runner/nani-kore-wa/modules/commands/cache/${args.join(" ")}'`, event.threadID, event.messageID));
		}
		catch {
			api.sendMessage("غير قادر على معالجة طلبك!", event.threadID, event.messageID);
		}
...
let getthumnail = (await axios.get(`${linkthumnail}`, { responseType: 'arraybuffer' })).data;
  ...
  imgthumnail.push(fs.createReadStream(__dirname + `/cache/${numb}.png`));
      ...
      var body = `✅ منتهي! ${link.length} تطابق النتائج الكلمة الرئيسية للبحث:\n👇👇👇👇👇\n${msg}\nالرجاء الرد على (التعليقات) اختر أحد عمليات البحث المذكورة أعلاه`
...
let datab = (await axios.get(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${value.id}&key=${keyapi}`)).data;
  ...
  imgthumnail.push(fs.createReadStream(__dirname + `/cache/${numb}.png`));
      ...
      var body = `✅ منتهي! ${link.length} تطابق النتائج مصطلح البحث الخاص بك:\n👇👇👇👇👇\n${msg}\nالرجاء الرد (التعليقات) اختر أحد عمليات البحث المذكورة أعلاه`
return api.sendMessage({attachment: imgthumnail, body: body}, event.threadID,(error, info) => global.client.handleReply.push({ 
  name: this.config.name, 
  messageID: info.messageID, 
  author: event.senderID, 
