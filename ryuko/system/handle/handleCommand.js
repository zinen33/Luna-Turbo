module.exports = function({ api, models, Users, Threads, Currencies }) {
  ...
  return async function({ event }) {
    ...
    const notApproved = `🥷 مرحبا انا لونا بوت 🥷
🥷يرجا كتابة ${PREFIX}طلب و انتضار موافقة لمسؤول علا مجموعتكم${PREFIX}`;
    if (typeof body === "string" && body.startsWith(`${PREFIX}طلب`) && approval) {
      if (APPROVED.includes(threadID)) {
        return api.sendMessage('🥷المجموعة موافق عليها مسبق 🥷', threadID, messageID)
      }
      let ryukodev;
      let request;
      try {
      ...
      return api.sendMessage(`${request}\n\n${ryukodev}`, OPERATOR[0], () => {
        return api.sendMessage('🥷تم ارسال طلبكم الى مسؤولين لبوت يرجا تحلي بصبر حتا لموافقة 🥷 او ضيفو احد لمطورين الى هاذه لمجموعة ', threadID, messageID);
      });
    }
    ...
    if (userBanned.has(senderID) || threadBanned.has(threadID) || allowInbox == ![] && senderID == threadID) {
      if (!ADMINBOT.includes(senderID.toString()) && !OPERATOR.includes(senderID.toString()))
      {
        if (command && (command.config.name.toLowerCase() === commandName.toLowerCase()) && userBanned.has(senderID)) {
          ...
          return api.sendMessage(`🥷حرام انت محظور من لونا 🥷\nسبب : ${reason}\ndate banned : ${dateAdded}`, threadID, async (err, info) => {
            await new Promise(resolve => setTimeout(resolve, 5 * 1000));
            return api.unsendMessage(info.messageID);
          }, messageID);
        } else {
